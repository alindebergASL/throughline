...
npm exec -- pnpm test:security with explicit DSNs:
  PASS — DB 13/13, authorization 14/14, Turbo 5/5
```

The 13-test PostgreSQL DB suite now proves legacy app-role login/password cleanup before explicit
...
same-name adoption constraint, app-role identity, RLS isolation, pooled context cleanup, and
Teams-subject rejection. The full root and security gates passed after the final second-pass edits.
...
fresh full local root and 27-test PostgreSQL-backed security gate. PR #2 remains held pending a new
