...
describe.sequential("durable truth command boundary", () => {
  ...
  it("revalidates live persisted evidence before returning an exact fact.accept replay", async () => {
    const { pool } = transactionPool(true);
    vi.spyOn(TruthLedgerRepository.prototype, "getSubjectScope").mockResolvedValue({
      subjectType: "activity",
      subjectId: id("4"),
    ...
    vi.spyOn(TruthLedgerRepository.prototype, "lockClaimSupportHeaders").mockResolvedValue([
      {
        claimId: id("10"),
    ...
    vi.spyOn(TruthLedgerRepository.prototype, "loadClaimsForAcceptance").mockResolvedValue([
      {
        claim: {
          id: id("10"),
          version: 2,
    ...
    await expect(
      new TruthLedgerDomainCommandBus(pool, authorization(true)).execute(factCommand(), context())
    ).resolves.toEqual({
      factId: id("13"),
      version: 1,
      status: "current",
